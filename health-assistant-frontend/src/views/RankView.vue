    ...
    <div class="lg:col-span-2">
      ...
      <div class="space-y-3">
        <div v-for="item in rankings" :key="item.userId"
          ...
          <div @click="showUserDetail(item)" class="w-10 h-10 rounded-full bg-white dark:bg-slate-800 flex items-center justify-center font-bold text-slate-700 dark:text-white shadow-sm mr-4 cursor-pointer hover:ring-2 hover:ring-green-400 transition-all">{{ (item.nickname||'?')[0] }}</div>
  ...
  <div v-if="selectedUser" class="fixed inset-0 z-[60] flex items-center justify-center px-4">
    ...
    <div class="bg-surface-light dark:bg-surface-dark w-full max-w-sm rounded-3xl p-6 relative z-10 shadow-2xl modal-enter border border-slate-100 dark:border-slate-800">
      ...
      <div class="text-center mt-4">
        <div class="w-20 h-20 mx-auto rounded-full bg-slate-100 dark:bg-slate-800 border-4 border-white dark:border-slate-700 shadow flex items-center justify-center text-2xl font-bold text-slate-600 dark:text-slate-300 mb-3">{{ (selectedUser.nickname||'?')[0] }}</div>
